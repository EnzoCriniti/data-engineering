...
| [14](./14-iac-cloud-localstack) | Infra como código (Terraform/LocalStack) | 🟡 Ambiente base | Provisiona o storage do lake como código, validado contra nuvem emulada (custo zero). |
| [15](./15-qualidade-contratos-dados) | Qualidade e contratos de dados | 🟡 Ambiente base | Portão de qualidade que valida o dado cru na borda antes das camadas confiáveis. |
| [16](./16-observabilidade-pipeline) | Observabilidade do pipeline | 🟡 Ambiente base | Mede freshness e volume das tabelas e alerta quando um SLO é violado. |
| [17](./17-capstone-consolidacao) | Capstone de consolidação | 🟢 Integração implementada | Amarra a plataforma inteira numa história única, com arquitetura consolidada e galeria de provas. |
...
8. Por fim, a plataforma vira produção: infra como código, qualidade/contratos, observabilidade — e um capstone que amarra tudo numa história única e demonstrável.
...
| Produção | 14-16 | Infra como código, qualidade/contratos e observabilidade do pipeline. |
| Consolidação | 17 | Arquitetura consolidada e galeria de provas da plataforma. |
...
| 14 IaC | plan/apply, idempotência e estado versionado | infra criada à mão não é reproduzível nem revisável. |
| 15 Qualidade | contratos de dados e validação na borda | fonte muda payload em silêncio e contamina dashboards. |
| 16 Observabilidade | freshness, volume e SLOs com alerta | job que falha sem erro serve dado velho ou incompleto. |
...
- [ ] `14` — aplicar o Terraform do lake contra o LocalStack.
- [ ] `15` — rodar o portão de qualidade sobre as fontes cruas.
- [ ] `16` — rodar o coletor de observabilidade e calibrar os SLOs.
- [ ] `17` — montar a galeria de provas e o diagrama consolidado.
...
Os capítulos 14–16 já trouxeram os controles de produção que antes eram só roadmap: infra como código (14), qualidade/contratos de dados (15) e observabilidade com SLOs e alertas (16). O capstone (17) consolida tudo.

O que ainda deixaria a plataforma mais próxima do dia a dia de um time pleno:
...
- lineage operacional automatizado entre fontes, jobs e tabelas;
- catálogo de dados mais completo, com ownership e descrição de tabelas;
- testes end-to-end dos pipelines no CI, não apenas validação de sintaxe;

...
> Guia rápido para **subir e usar**. Status atual: **ambiente base / documentação** — o Postgres de features sobe pronto com o schema `ml`; a carga das features a partir das fontes é o roteiro do [GUIDE.md](./GUIDE.md) (código no [SOLUTION.md](./SOLUTION.md)).
...
```bash
docker compose exec db psql -U features -d features -c "\d ml.fraude_pagamento_features"
```
...
# inspecionar
docker compose exec db psql -U features -d features -c \
